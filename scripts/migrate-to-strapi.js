...
const STRAPI_URL = 'http://127.0.0.1:1337'; // IPv4 conseillé
...
const phones = [
  ...
  },
  {
    ...
    isActive: true,
    images: ["real/Nokia G60/front-back-one.jpg"]
  },
  {
    name: "Realme 10",
    brand: "Realme",
    ...
    isActive: true,
    images: ["real/Realme 10/front.jpg"]
  }
];

async function uploadImage(imagePath) {
  try {
    const fullPath = path.join(UPLOAD_PATH, imagePath);
    ...
    return res.data[0].id; // retourne l'id de l'image uploadée
  } catch (error) {
  ...
  try {
    ...
    if (error.response) {
      if (error.response.status === 403 || error.response.status === 405) {
        console.error('Erreur de permission : Configurez "create" sur Phone et "uploadFiles" sur Upload dans Strapi Admin.');
      } else {
