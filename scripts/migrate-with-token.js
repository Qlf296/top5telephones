// scripts/migrate-with-token.js
const axios = require('axios');

const STRAPI_URL = 'http://127.0.0.1:1337';
// Remplacez par votre token API créé dans Strapi Admin
...
async function createPhone(phone) {
  try {
    ...
    const res = await axios.post(`${STRAPI_URL}/api/phones`, phoneData, {
      headers: {
...
(async () => {
  if (API_TOKEN === 'YOUR_API_TOKEN_HERE') {
    console.log('⚠️  Configurez votre token API :');
    console.log('1. Allez dans Strapi Admin → Settings → API Tokens');
    console.log('2. Créez un nouveau token avec "Full access"');
    console.log('3. Copiez le token et remplacez YOUR_API_TOKEN_HERE dans ce script');
    return;
  }
  
  console.log('Migration avec token API démarrée...');
